...
|---|---|
| Automated test passes | `npm test` — 22/22, output in [Testing](#testing) |
| **User A cannot access User B's contacts** | `npm run test:rls` — 7/7 against the live database, plus the screenshot below |
| Deployed app works end to end | Production smoke test — 6/6, output in [Testing](#testing) |
| Sign-in and sign-out | screenshots below |
| Create, edit, delete, refresh a contact | screenshots below |
| Invalid input fails safely | screenshot below |
| Schema and RLS ownership explained | [Authentication and RLS ownership](#authentication-and-rls-ownership) |
| No committed secret values | `.env.example` holds placeholders only; `.env.local` and `.rls-test.local` are git-ignored and absent from history |

All screenshots are generated by [`scripts/screenshots.mjs`](scripts/screenshots.mjs):

```bash
npm run screenshots            # against http://localhost:3000
npm run screenshots -- --prod  # against the deployed app
```

It resets both test accounts to an empty state first, so the images are
reproducible rather than a snapshot of whatever happened to be in the database.

### Sign in

![Sign-in form](docs/screenshots/01a-sign-in-form.png)

Signed in, showing the account and the sign-out control:

![Signed in](docs/screenshots/01b-signed-in.png)

### Create, view, and persist contacts

![Contacts created](docs/screenshots/02a-contacts-created.png)

The same list after a full browser refresh — the data lives in Neon Postgres, not in component state:

![After refresh](docs/screenshots/02b-after-refresh.png)

### Invalid input fails safely

Submitting a blank name. The message comes from server-side Zod validation, and the database
enforces the same rule with `check (length(btrim(name)) > 0)` for callers who skip the API:

![Blank name rejected](docs/screenshots/03-validation-blank-name.png)

### User B cannot see User A's contacts

Same application, same database, same moment — a different account. User A has two contacts;
User B sees none of them. No filtering in application code produces this: it is
`contacts_select_own` refusing the rows.

![User B sees nothing](docs/screenshots/04-user-b-empty.png)

The [RLS suite](#live-row-level-security-verification) proves the stronger form of this, attacking
the public Data API directly rather than going through the UI.

### Mobile layout

The list renders as stacked cards below the `sm` breakpoint and as a table above it:
